 ...
 *
 * Hard (31.29%)
 * Total Accepted:    12642
 * Total Submissions: 40376
...
public:
    bool canCross(vector<int>& stones) {
        ...
        steps[stones[0]].insert(0);
        int x = stones.back();

        for (int i = 0; i < n; ++i) {
            int j = stones[i];
            for (auto s : steps[j]) {
                if (j+s-1 == x) return true;
                if (s > 1) steps[j+s-1].insert(s-1);

                if (j+s == x) return true;
                if (s) steps[j+s].insert(s);

                if (j+s+1 == x) return true;
                steps[j+s+1].insert(s+1);
            }
        }
        return false;
    }
