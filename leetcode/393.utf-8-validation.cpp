 ...
 *
 * Medium (34.71%)
 * Total Accepted:    11486
 * Total Submissions: 33133
...
class Solution {
public:
    bool check(unsigned x, int plen) {
        bool b = true;
        switch (plen) {
            case 1: b = ((x >> 7) == 0b0); break;
            case 2: b = ((x >> 6) == 0b10); break;
            case 3: b = ((x >> 5) == 0b110); break;
            case 4: b = ((x >> 4) == 0b1110); break;
            case 5: b = ((x >> 3) == 0b11110); break;
        }
        return b;
    }
    ...
    bool validUtf8(vector<int>& data) {
        ...
        while (i < n) {
            int m = 0;
            if (check(data[i], 5)) m = 3;
            else if (check(data[i], 4)) m = 2;
            else if (check(data[i], 3)) m = 1;
            else if (check(data[i], 1)) m = 0;
            else return false;

            while (m) {
                ++i; --m;
                if (!check(data[i], 2)) return false;
            }
            ++i;
        }
        return true;
