...
public:
    bool PredictTheWinner(vector<int>& nums) {
        ...
        for (auto &v : M) v.resize(n, 0);
        for (int i = 0; i < n; ++i) M[i][i] = nums[i];

        for (int i = n-1; i >= 0; --i)
            for (int j = i; j < n; ++j) {
                int sum = accumulate(&nums[i], &nums[j+1], 0);

                if (i < n-1) M[i][j] = max(M[i][j], sum - M[i+1][j]);
                if (j > 0) M[i][j] = max(M[i][j], sum - M[i][j-1]);
            }
